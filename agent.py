...
class Agent(core.Actor, core.VariableSource):
  ...
  def select_action2(self, mask: np.array, observation: types.NestedArray) -> types.NestedArray:
    return self._actor.select_action2(mask, observation)
  ...
  def observe2(self, mask: np.array, action: types.NestedArray, next_timestep: dm_env.TimeStep):
    self._num_observations += 1
    self._actor.observe2(mask, action, next_timestep)
